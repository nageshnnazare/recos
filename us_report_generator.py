...
def fmt_val(val):
    ...
    if val is None or math.isnan(val) if isinstance(val, (int, float)) else False:
        return "N/A"
...
def generate_price_chart_svg(hist_data, width=1040, height=280):
    ...
    events.append((len(prices) - 1, f"CMP: ${prices[-1]:.1f}", "#3d9cf5") if prices[-1] < 100 else (len(prices) - 1, f"CMP: ${prices[-1]:.0f}", "#3d9cf5"))
...
def generate_fair_value_svg(current_price, fair_value_low, fair_value_mid, fair_value_high, width=480, height=90):
    ...
    svg += f'  <text x="{xc:.1f}" y="8" text-anchor="middle" font-family="Fira Code,monospace" font-size="10" font-weight="700" fill="{color}">{fmt_val(current_price)} CMP</text>\n'
    ...
    if qi is not None and not qi.empty:
        for col in qi.columns[:8]:  # last 8 quarters max
            try:
                ...
                month = dt.month
                year = dt.year
                # Determine US quarter based on month
                q_label = f"Q{(month-1)//3 + 1} '{year % 100}"
...
def generate_html_report(data, scores):
    ...
    for q in quarterly_rows:
        rev_str = fmt_val(q['revenue']) if q['revenue'] else "N/A"
        profit_str = fmt_val(q['profit']) if q['profit'] else "N/A"
        ebitda_str = f"{q['ebitda_margin']:.1f}%" if q['ebitda_margin'] else "N/A"

        rev_class = "tg" if q['revenue'] and q.get('_prev_rev') and q['revenue'] > q['_prev_rev'] else ""
        profit_class = "tg" if q['profit'] and q['profit'] > 0 else "tr" if q['profit'] and q['profit'] < 0 else ""
    ...
    ev_revenue = market_cap / total_revenue if total_revenue else 0
    ...
    if upside > 10:
        catalysts.append(f"<strong>Analyst Upside ({upside:.0f}%):</strong> Mean target of {fmt_val(target_mean)} above current price.")
    ...
    verdict_text = f'''<strong>{company_name}</strong> trades at {fmt_val(current_price)} with a composite risk score of {composite}/100.
    ...
    <br><br>
    {"Analyst consensus suggests upside of " + f"{upside:.1f}%" + f" with a mean target of {fmt_val(target_mean)}." if target_mean and upside > 0 else "The stock is trading near or above analyst consensus targets."}
...
<title>{ticker_symbol} · Risk Score Report</title>
...
<div class="page">
  ...
  <div class="stock-header">
    <div>
      <div class="sh-ticker"><span>{ticker_symbol}</span><span class="badge badge-nse">US</span><span class="badge badge-sector">{sector}</span></div>
      <div class="sh-name">{company_name}</div>
      <div class="sh-meta"><span>📊 {industry}</span><span>📍 52W: {fmt_val(low_52w)} — {fmt_val(high_52w)}</span></div>
    </div>
    <div class="sh-right">
      <div class="sh-price">{fmt_val(current_price)}</div>
      <div class="sh-change" style="color:{change_color}">{change_icon} {fmt_val(abs(change))} ({change_pct:+.2f}%)</div>
  ...
  <div class="gauge-kpi-row">
    ...
    <div class="kpi-strip">
      ...
      <div class="kpi-card"><div class="kpi-label">💰 EPS</div><div class="kpi-value">{fmt_val(eps)}</div><div class="kpi-sub">TTM</div></div>
      <div class="kpi-card"><div class="kpi-label">🏛 MARKET CAP</div><div class="kpi-value">{fmt_val(mcap_val)}</div><div class="kpi-sub">{"Large" if mcap_val > 10e9 else "Mid" if mcap_val > 2e9 else "Small"} Cap</div></div>
  ...
  <div class="section">
    ...
    <div class="card-grid">
      ...
      <div class="metric-card {"beat" if target_mean > current_price else "miss" if target_mean else "caution"}"><div class="mc-label">ANALYST TARGET</div><div class="mc-value">{fmt_val(target_mean)}</div><div class="mc-bench">Range: {fmt_val(fair_value_low)} - {fmt_val(fair_value_high)}</div></div>
    </div>
  </div>
  ...
  <div class="breakdown-grid">
    <div class="breakdown-card">
      ...
      <ul class="bc-items">
        ...
        <li>Analyst target: {fmt_val(target_mean)} ({upside:+.1f}%)</li>
  ...
  <div class="footer">
    RISK SCORE REPORT · {ticker_symbol} · GENERATED {datetime.now().strftime("%b %d, %Y %H:%M ET").upper()} · DATA VIA YAHOO FINANCE<br>
...
def generate_alerts_summary(results, output_dir, date_str=None):
    ...
    for r in results:
        ...
        row = f"| {ticker} | {fmt_val(current)} | {scores['composite']}/100 | {signal} | {upside:+.1f}% | [Report](./{today}/{ticker}_RiskReport.html) |"
        all_rows.append(row)
...
Examples:
  ...
  python report_generator.py GROWW -o ./reports/
    ...
    parser.add_argument("tickers", nargs="*", help="US ticker symbol(s) (e.g. AAPL, MSFT)")
    parser.add_argument("-w", "--watchlist", help="Path to US watchlist file")
    ...
    if args.alerts or len(tickers) > 1:
        ...
        if alerts:
            ...
            for a in alerts:
                print(f"     🟢 {a['ticker']} — {fmt_val(a['price'])} (Score: {a['score']}, Upside: {a['upside']:+.1f}%)")
        else:
